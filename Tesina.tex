...
Rappresenta il rapporto fra le forze inerziali e quelle viscose: per capirlo, scegliamo un volume (come un cilindro) di superficie $S$ e lunghezza $L$, e un tempo unitario $t$ (interpretando dunque le derivate come semplici divisioni per tale intervallo) possiamo esprimerle come
...
Chiaramente, questo calcolo non è formalmente impeccabile, ma permette di capire il concetto dietro a questo numero.
...
La turbolenza avviene in quello che è detto \emph{inertial range}, ovvero l'insieme di scale per le quali dominano le forze inerziali: fra $d_{\text{max}}$, la dimensione del sistema, e $d_{\text{min}}$, la scala alla quale avviene la dissipazione in calore. Kolmogorov ha postulato che quest'ultima dipenda solo da $\epsilon$ e $\nu$. Dimensionalmente, possiamo trovare gli esponenti di questi parametri:
